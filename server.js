...
// Railway requires an HTTP server on the port for health checks.
// The WebSocket server is attached to it so both share the same port.
const server = http.createServer((req, res) => {
  res.writeHead(200, { 'Content-Type': 'text/plain' });
  res.end('RoadLink signaling server OK');
});

const wss = new WebSocketServer({ server });

server.listen(PORT, () => {
  console.log(`[RoadLink Server] Listening on port ${PORT}`);
});
